...
def text():
    ...
    while True:  # User can add or remove concepts
        try:
            concept_change = raw_input(
                "Would you like to [add] or [delete] a main concept?  Or would you like to [skip] this step?")
    ...
    while True:
        try:
            sensitivity = input(
                "How far ahead would you like the program to check for linking concepts?  (2-Low 4-Normal 6-High)")
            if sensitivity > 0:
                break
            else:
    ...
    while True:
        ...
        if save_format == "PNG" or save_format == "png":
            ...
            for x in range(0, concept_connection_counter):  # Adds concepts to file
                file.write(
                    concept_connections[x][0] + chr(9) + concept_connections[x][1] + chr(9) + concept_connections[x][
                        2] + "\n")
    ...
    print("The student had " + str(similar_prop) + " propositions in common with the gold standard of " + str(
        number_of_master_prop) + " propositions.")
